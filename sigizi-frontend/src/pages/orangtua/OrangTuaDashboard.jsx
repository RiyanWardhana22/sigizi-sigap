...
export default function OrangTuaDashboard() {
  ...
  const generateNotifications = (anakListData) => {
      ...
      if (lastMeasurement) {
        if (lastMeasurement.status_gizi === "Stunting") {
          notif.push({
            ...
            message:
              "Terindikasi stunting. Segera konsultasikan ke posyandu terdekat!",
            type: "danger",
          });
        ...
        if (
          secondLast &&
          parseFloat(lastMeasurement.berat_badan) <
            parseFloat(secondLast.berat_badan)
        ) {
          notif.push({
            id: `weight-${anak.id}`,
            title: `${anak.nama_anak}`,
    ...
    notif.push({
      ...
      message:
        "Anak usia 0-12 bulan membutuhkan imunisasi dasar lengkap. Cek ke posyandu terdekat!",
      type: "info",
    });
  ...
  const handleAnakChange = (anakId) => {
    ...
    if (selectedAnak) {
      ...
      if (selectedAnak.riwayat && selectedAnak.riwayat.length > 0) {
        setLatestMeasurement(
          selectedAnak.riwayat[selectedAnak.riwayat.length - 1],
        );
      } else {
        setLatestMeasurement(null);
      }
  ...
  const statusColor =
    latestMeasurement?.status_gizi === "Normal"
      ...
      : latestMeasurement?.status_gizi === "Stunting"
        ? "text-red-600"
        : latestMeasurement?.status_gizi === "Pra-stunting"
          ? "text-yellow-600"
          : "text-gray-600";
              ...
              <div className="flex justify-between items-start flex-wrap gap-4">
                <div>
                  ...
                  <p className="opacity-90 mt-1">
                    Lahir: {selectedAnak.tanggal_lahir} | JK:{" "}
                  ...
                  {latestMeasurement && (
                    <div className="flex flex-wrap gap-3 mt-3">
                      ...
                      <div
                        className={`flex items-center gap-2 bg-white/20 rounded-lg px-3 py-1.5 ${statusColor}`}
                      >
                        <FontAwesomeIcon icon={fas.faChartLine} />
                        <span className="font-medium">
                          {latestMeasurement.status_gizi}
                        </span>
                      </div>
                    </div>
                  )}
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              {growthData.length > 0 ? (
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  />
                  <p>
                    Belum ada data pengukuran untuk{" "}
                    {selectedAnak?.nama_anak || "anak ini"}
                  </p>
                </div>
              )}
            </div>
